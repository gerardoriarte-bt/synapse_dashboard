> ⚠️ **HISTÓRICO · no usar para planificar.** Escrito el 2026-09-04, **antes de
> que el backend de la consola existiera**. Varias cosas que declara como
> faltantes —las ocho rutas, `POST /auth/login`, las seis de admin y builder— ya
> están servidas desde el 2026-09-11.
>
> Se conserva porque el razonamiento de cada pedido sigue siendo válido y
> explica por qué se pidió. **Lo vigente se genera**: `docs/PARA-BACKEND.md`
> sale de `plan-de-trabajo.md` en cada `npm run plan`.
